    ...
    if test_flag == 0:
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 1 == 0:
                ...
                if rmse < best_rmse:
                    ensemble_path = fr'F:\soh\first\results\conventional_experiment\{condition}\{2*epochs}_{seed}_{ci}_result\ensemble_weights.pth'
                    best_rmse = rmse
...
def trainbase(batch_size,epochs,model,train_data,loss_fn, optimizer,valid_data,test_data,test_flag,model_flag,ci,seed):
    ...
    test_loader = DataLoader(test_data, batch_size=batch_size, shuffle=False)
    rmses = []
    base_path = fr'F:\soh\first\results\conventional_experiment\{condition}\{epochs}_{seed}_{ci}_result\{model_flag}_weights.pth'
    if test_flag == 0:
        best_rmse = np.inf
        for epoch in range(epochs):
            model.train()
