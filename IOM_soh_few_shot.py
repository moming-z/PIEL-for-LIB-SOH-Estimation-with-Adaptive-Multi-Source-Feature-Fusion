#---------------------------------------#
#训练集减少到两个电芯数据，测试集与主实验保持一致，从零开始训练
import os
import numpy as np
import torch
#--------------------------------------#
...
#--------------------------------------#
#模型结构
from model.model_qianyi2 import *
#--------------------------------------#
#后处理
from first.results.qianyi_experiment2.houchuli_qianyi2 import *
...
for ci in range(cishu):
    os.makedirs(fr'F:\soh\first\results\qianyi_experiment2\{source_condition}__{condition}\{epochs}_{seed}_{ci}_result', exist_ok=True)
    os.makedirs(fr'F:\soh\first\results\qianyi_experiment2\{source_condition}__{condition}\{epochs}_{seed}_metrics', exist_ok=True)
    ...
    model_flag = 'cnn'
    if test_flag == 1:
        weights_path = rf'F:\soh\first\results\qianyi_experiment2\{source_condition}__{condition}\{epochs}_{seed}_{ci}_result\cnn_weights.pth'
    ...
    mlp = MLP(x_dim,hidden_dim,out_dim)
    if test_flag == 1:
        weights_path2 = rf'F:\soh\first\results\qianyi_experiment2\{source_condition}__{condition}\{epochs}_{seed}_{ci}_result\mlp_weights.pth'
    ...
    mlp.load_state_dict(torch.load(rf'F:\soh\first\results\qianyi_experiment2\{source_condition}__{condition}\{epochs}_{seed}_{ci}_result\mlp_weights.pth',weights_only=True))
    cnn.load_state_dict(torch.load(rf'F:\soh\first\results\qianyi_experiment2\{source_condition}__{condition}\{epochs}_{seed}_{ci}_result\cnn_weights.pth',weights_only=True))
    ...
    os.makedirs(fr'F:\soh\first\results\qianyi_experiment2\{source_condition}__{condition}', exist_ok=True)
    np.save(fr'F:\soh\first\results\qianyi_experiment2\{source_condition}__{condition}\{epochs}_{seed}_{ci}_result\true-pre.npy',temp)

np.save(fr'F:\soh\first\results\qianyi_experiment2\{source_condition}__{condition}\{epochs}_{seed}_metrics\statistic.npy',smetrics)
np.save(fr'F:\soh\first\results\qianyi_experiment2\{source_condition}__{condition}\{epochs}_{seed}_metrics\physics.npy',xmetrics)
np.save(fr'F:\soh\first\results\qianyi_experiment2\{source_condition}__{condition}\{epochs}_{seed}_metrics\ensemble.npy',enmetrics)
