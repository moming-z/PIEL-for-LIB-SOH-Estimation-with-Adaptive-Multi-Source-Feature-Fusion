...
def huitu(y_pre,y_true,losslist,epochs,valid_rmses,flag,test_flag,condition,model_flag,ci ,seed):
    ...
    plt.legend()
    # plt.show()
    plt.savefig(rf'F:\soh\first\results\ablation_experiment\{condition}\{epochss}_{seed}_{ci}_result\{model_flag}_pre-true.png')
    ...
    if test_flag == 0:
        ...
        plt.rcParams['axes.unicode_minus'] = False  # 解决负号显示问题
        plt.legend()
    plt.savefig(rf'F:\soh\first\results\ablation_experiment\{condition}\{epochss}_{seed}_{ci}_result\{model_flag}_loss.png')
...
if __name__ == "__main__":
    condition = 'NCACY25-05_1'
    # condition = 'LCO'
    edata = np.load(fr'F:\soh\first\results\ablation_experiment\{condition}\1500_100_metrics\ave.npy')
    pdata = np.load(fr'F:\soh\first\results\ablation_experiment\{condition}\1500_100_metrics\end2end.npy')
    print(f'直接平均：{edata}')
    print(f'端到端：{pdata}')
