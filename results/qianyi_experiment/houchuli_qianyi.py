...
def huitu(y_pre,y_true,losslist,epochs,valid_rmses,flag,test_flag,condition,model_flag,ci ,seed):
    ...
    plt.legend()
    # plt.show()
    plt.savefig(rf'F:\soh\first\results\qianyi_experiment3\{source_condition}__{condition}\{epochs}_{seed}_{ci}_result\{model_flag}_pre-true.png')
    ...
    if test_flag == 0:
        ...
        plt.rcParams['axes.unicode_minus'] = False  # 解决负号显示问题
        plt.legend()
        plt.savefig(rf'F:\soh\first\results\qianyi_experiment3\{source_condition}__{condition}\{epochs}_{seed}_{ci}_result\{model_flag}_loss.png')
